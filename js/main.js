...
//get seartch input
  ...
  function scrollSmoothToBottom () {
    $(scrollingElement).animate({
     scrollTop: $("#pokeprofile").offset().top
      }, 300);
     }
    ...
    requests.onload = function(e){
      ...
      if(requests.readyState === 4 ){
        ...
        if(requests.status === 200){
          ...
          //Get 5 moves and write the HTML for moves
          moves = response.moves[0];
          console.log("move length",response.moves.length);
            //if pokémon has less than 5 moves
            if(response.moves.length < 5){
              console.log("less than 5moves");
              ...
              for(var i=response.moves.length;i<5;i++){
                newid = "move".concat(i);
                document.getElementById(newid).innerHTML = " ";
              }
            }
            //if pokémon has 5 moves or more
            else{
              for (var i=0;i<5;i++){
                listtwo.push(response.moves[i].move.name);
              }
          ...
          //get picture and write HTML for the picture
          if (pokeid < 10) {
          ...
          for (var i=0;i<typeTotal;i++){
            ...
            console.log("type names",listType[i]);
          }
          //get ability
